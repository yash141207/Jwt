...
# OUTPUT FORMAT: { "uid": <int>, "token": "<jwt>" }
...
def rizer_endpoint():
    """Single token processing - Returns {uid, token}"""
    ...
    for pt in platform_types:
        ...
        if result:
            ...
            if jwt_decoded and jwt_decoded[0]:
                uid_int = int(jwt_decoded[0])
                return jsonify({
                    "uid": uid_int,
                    "token": result['token']
                })
...
def bulk_endpoint():
    """Bulk process tokens from JSON body - Returns [{uid, token}]"""
...
def bulk_tokens_from_file():
    """Upload tokens JSON file and process - Returns [{uid, token}]"""
...
def bulk_uidpass():
    """Bulk process UID:PASS pairs - Returns [{uid, token}]"""
    data = request.get_json(silent=True)
...
def status_endpoint():
    ...
    return jsonify({
        ...
        "endpoints": {
            "/rizer": "GET - Single token (query param: access_token) -> {uid, token}",
            "/bulk": "POST - Bulk tokens JSON body -> [{uid, token}]",
            "/bulk/tokens": "POST - Upload tokens file -> [{uid, token}]",
            "/bulk/uidpass": "POST - Bulk UID:PASS -> [{uid, token}]",
...
def home():
    return jsonify({
        ...
        "version": RIZER,
        "author": "@beotherjkman",
        "output_format": {"uid": "<int>", "token": "<jwt>"},
        "endpoints": {
            "/rizer": "GET - Single token -> {uid, token}",
            "/bulk": "POST - Bulk tokens -> [{uid, token}]",
            "/bulk/tokens": "POST - Upload file -> [{uid, token}]",
            "/bulk/uidpass": "POST - UID:PASS -> [{uid, token}]",
            "/status": "GET - API status"
        },
        "docs": "All endpoints now return uid (account_id) + jwt token only"
    })
